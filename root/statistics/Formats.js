        ...
        <tbody>
          ...
          {formatStats.map((formatStat, i) => (
            <tr className={(i + 1) % 2 === 0 ? 'even' : 'odd'} key={formatStat.medium_stat}>
              ...
              <td className="t">{formatPercentage(formatStat.release_count / stats.data['count.release'])}</td>
              <td className="t">{formatCount(formatStat.medium_count)}</td>
              <td className="t">{formatPercentage(formatStat.medium_count / stats.data['count.medium'])}</td>
            </tr>
          ))}
        </tbody>
      </table>
